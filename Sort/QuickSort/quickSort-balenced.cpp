...
void quickSort( vector< int > &num ) {
	...
	if ( num.empty() )
		return;
	// choose pivot from (beg,mid,end)
	int mid = num.size() / 2;
	// choose the middle value
	int pivot_idx = 0;
	if ( ( num[ 0 ] >= num[ mid ] && num[ mid ] >= num.back() ) || ( num.back() >= num[ mid ] && num[ mid ] >= num[ 0 ] ) )
		pivot_idx = mid;
	else if ( ( num[ 0 ] >= num.back() && num.back() >= num[ mid ] ) || ( num[ mid ] >= num.back() && num.back() >= num[ 0 ] ) )
		pivot_idx = num.size() - 1;
	// set pivot
